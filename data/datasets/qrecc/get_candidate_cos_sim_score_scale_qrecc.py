...
DATA_PATH='/itercqr/data/new_preprocessed/train_with_doc.json'
...
embs = torch.load('/itercqr/data/new_preprocessed/train_pos_docs_emb.pt')
min=1
max=0
out = []
score_list=[]
with open(OUTPUT_PATH, 'w') as fw:
    for i in tqdm(range(len(data))):
        ...
        if embs[example_id]==[]:
            ...
            if len(pos_docs_emb)>1:
                scores_=[]
                for pos_emb in pos_docs_emb:
                    scores_.append(torch.nn.functional.cosine_similarity(candidates_emb, pos_emb))
                scores = torch.max(torch.stack(scores_), dim=0).values
                #scores = torch.mean(torch.stack(scores_), dim=0)
