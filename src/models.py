...
from customANCE import AnceEncoder
...
class ANCE(RobertaForSequenceClassification):
    ...
    def query_emb(self, inputs_embeds,attention_mask ):
        outputs1 = self.roberta(inputs_embeds=inputs_embeds,
    ...
    def forward(self, attention_mask, inputs_embeds, wrap_pooler=False):
        return self.query_emb( inputs_embeds, attention_mask)

# ANCE model
class ANCE_freeze(RobertaForSequenceClassification):
    # class Pooler:   # adapt to DPR
    #     def __init__(self, pooler_output):
    #         self.pooler_output = pooler_output
