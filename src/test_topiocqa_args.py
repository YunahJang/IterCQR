...
from data.preprocess_topiocqa import load_collection
...
def output_test_res(query_embedding2id,
    ...
    output_trec_file = oj(args.qrel_output_path, "ANCE_t5_rewrite+simq+answer_res.trec")
    merged_data = []
    ...
    if args.result_save_path is not None:
        with open(args.result_save_path, 'a') as f:
            f.write('TOPIOCQA DENSE' + '\n')
            f.write(str(trec_res) + '\n\n')
    return trec_res
...
def gen_metric_score_and_save(args, index, query_embeddings, query_embedding2id):
    ...
    trec_res = output_test_res(query_embedding2id,
...
def main():
    ...
    trec_res = gen_metric_score_and_save(args, index, query_embeddings, query_embedding2id)

    logger.info("Test finish!")
    return str(trec_res)


def evaluate(test_file_path, qrel_output_path, trec_gold_qrel_file_path):
    ...
    args.query_encoder_checkpoint="castorini/ance-msmarco-passage"
    args.pretrained_passage_encoder="castorini/ance-msmarco-passage"
    args.max_concat_length=512
    args.max_query_length=64
    args.max_doc_length=384
    args.max_response_length=64
    args.seed=42
    args.model_type="ANCE"
    args.passage_block_num=26
    args.per_gpu_eval_batch_size=2
    args.batch_size=2
    args.n_gpu=1
    args.use_gpu=True
    args.top_n=100
    args.use_data_percent=1
    args.rel_threshold=1
    args.cross_validate=False
    args.disable_tqdm=False
    args.use_last_response=False
    args.use_wrong_last_response=False
    args.use_prefix=True
    args.use_PRF=False
    args.skip_all_zero=False
    args.PRF_mode='hard'
    args.test_type='rewrite'
    args.original=True
    args.eval_type='rewrite'
    args.collate_fn_type='flat_concat_for_train'
    args.test_file_path=test_file_path
    args.test_file_path_2=None
    args.trec_gold_qrel_file_path=trec_gold_qrel_file_path
    args.passage_collection_path="../data/datasets/topiocqa/full_wiki_segments.tsv"
    args.passage_embeddings_dir_path="../data/datasets/topiocqa/embeddings_customANCE"
    args.passage_offset2pid_path="../data/datasets/topiocqa/tokenized/offset2pid.pickle"

    args.qrel_output_path=qrel_output_path
    args.result_save_path=None
...
def get_args():
    ...
    parser.add_argument("--query_encoder_checkpoint", type=str, default="castorini/ance-msmarco-passage")
    ...
    parser.add_argument("--use_data_percent", type=int, default=1)
    ...
    parser.add_argument("--use_wrong_last_response", type=bool, default=False)
    parser.add_argument("--use_prefix", type=bool, default=True)
    parser.add_argument("--use_PRF", type=bool, default=False)
    parser.add_argument("--skip_all_zero", type=bool, default=False)
    parser.add_argument("--PRF_mode", type=str, default='hard')
    parser.add_argument("--test_type", type=str, default='rewrite')
    parser.add_argument("--original", type=bool, default=True)
    parser.add_argument("--eval_type", type=str, default='rewrite')
    ...
    parser.add_argument("--test_file_path_2", type=str, default=None)
    parser.add_argument("--trec_gold_qrel_file_path", type=str, default="../data/datasets/topiocqa/dev_gold.trec")

    parser.add_argument("--passage_collection_path", type=str, default="../data/datasets/topiocqa/full_wiki_segments.tsv")
    parser.add_argument("--passage_embeddings_dir_path", type=str, default="../data/datasets/topiocqa/embeddings_customANCE")
    parser.add_argument("--passage_offset2pid_path", type=str, default="../data/datasets/topiocqa/tokenized/offset2pid.pickle")

    parser.add_argument("--qrel_output_path", type=str, default=None)
