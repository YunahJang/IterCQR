...
def output_test_res(query_embedding2id,
    ...
    output_trec_file = oj(args.qrel_output_path, "ANCE_GPT_QR_res_rewrite_only.trec")
    # merged_data = []
    ...
    with open(output_trec_file, "w") as g:
        for qid, passages in qids_to_ranked_candidate_passages.items():
            ...
            for i in range(topN):
                ...
                # passage = all_passages[pid] if i < 10 else ''
                # rank_list.append(
                #     {
                #         "doc_id": str(pid),
                #         "rank": i+1,
...
def get_args():
    ...
    parser.add_argument("--only250", type=bool, default=False)
    ...
    parser.add_argument("--trec_gold_qrel_file_path", type=str, default="../data/new_preprocessed/qrecc_qrel.tsv")

    parser.add_argument("--passage_collection_path", type=str, default="../data/new_preprocessed/qrecc_collection.tsv")
    parser.add_argument("--passage_embeddings_dir_path", type=str, default="../data/datasets/ml-qrecc/embeddings_customANCE")
    parser.add_argument("--passage_offset2pid_path", type=str, default="../data/datasets/ml-qrecc/tokenized/offset2pid.pickle")

    parser.add_argument("--qrel_output_path", type=str, required=True)
    parser.add_argument("--qrel_output_name", type=str, required=True)
